...
bool Sock::waitSend(INT timeout)
{
	...
	FD_ZERO(&writeFD);
	FD_SET(*this,&writeFD);
	::select((int)*this+1,NULL,&writeFD,NULL,&it);
...
INT Sock::send2(const void* lpBuf, INT nBufLen, INT nFlags,INT timeout) const
{
	...
	while(size)
	{
		...
		if(FD_ISSET(*this,&writeFD))
		{
			INT ret = ::send(m_Sofd,(LPCSTR)lpBuf,nBufLen,nFlags);
			if(ret == -1)
				break;
			if(ret > 0)
				size -= size;
...
bool Socket::connect(LPCSTR pHost,INT iPort)
{
	...
	//スレッド設定
	m_bThreadValid = true;
//	m_aflThread.startThread(this,0);
...
DWORD Socket::onThreadProc(LPVOID pvData)
{
	...
	//接続動作完了の通知
	if(m_pcallConnect)
		m_pcallConnect->callProcess(this);
	...
	while(m_bThreadValid)
	{
		...
		else if(iSize > 0)
		{
			...
			if(m_pcallRecv)
				m_pcallRecv->callProcess(&sockData);
			onSocketRecv(iSize,m_pbyRecvBuff);
		}
	}
	return 0;
}
void Socket::setConnect(ClassCallBack* pCallBack)
{
	m_pcallConnect = pCallBack;
}
void Socket::setRecv(ClassCallBack* pCallBack)
{
	m_pcallRecv = pCallBack;
}
void Socket::setClose(ClassCallBack* pCallBack)
{
	m_pcallClose = pCallBack;
}
...
bool HttpData::read(LPCSTR urlAddr)
{
		...
		CHAR buff[4000];
		int size;
		while((size = socket.recv(buff,sizeof(buff),0,10000)) > 0)
		{
		...
		LPCSTR headerLast = strstr(data,"\r\n\r\n");
		if(!headerLast)
			return false;
		INT headerSize = (INT)(headerLast-data);
		m_header.assign(data,headerSize+2);
		m_body.assign(headerLast + 4,work.size()-headerSize-4);

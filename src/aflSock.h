...
public:
	...
	{
		BYTE b[6];
		sscanf(mac,"%02x%*c%02x%*c%02x%*c%02x%*c%02x%*c%02x",
...
public:
	...
	void setConnect(ClassCallBack* pCallBack);
	void setRecv(ClassCallBack* pCallBack);
	void setClose(ClassCallBack* pCallBack);
...
protected:
	...
	ClassCallBack* m_pcallConnect;
	ClassCallBack* m_pcallClose;
	ClassCallBack* m_pcallRecv;
	Thread m_aflThread;

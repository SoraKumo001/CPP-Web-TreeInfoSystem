...
bool WibsBase::init()
{
	...
	LPCSTR sql = 
		"BEGIN;"
		"CREATE TABLE tb_file2(file_id integer primary key,file_pid integer,file_kind,"
		"	file_name,file_lname,file_type,file_size  integer,file_date,file_hash,file_comment);"
		"CREATE TABLE tb_mfile(id integer primary key,enable integer,parent_id integer,name,data,option1,option2);"
		"CREATE TABLE tb_link(id integer primary key,message_id int,enable int,name,url);"
		"COMMIT;";
	m_sqlite.exec(sql);
	return true;
}
...
bool WibsBase::isEdit()
{
	...
	if(m_edit == -1)
	{
		LPCSTR methodMode = Method::COOKIE("mode");
...
void WibsBase::outUpload()
{
	...
	for(i=0;i<20;i++)
	{
		const MethodFile* methodFile = Method::FILE(NULL,i);
		if(!methodFile)
			break;
		file.saveFile(methodFile,pid);
	}
	LPCSTR script =
		"<SCRIPT>if(window.parent.WIBS.callbackUpload)window.parent.WIBS.callbackUpload();</SCRIPT>";
	m_stream->out(script);

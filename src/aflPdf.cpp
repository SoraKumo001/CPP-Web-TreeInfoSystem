...
INT getImageWord(FILE* file)
{
	BYTE work[2];
	if(fread(work,2,1,file)==0)
		return 0;
	return (work[0] << 8) + work[1];
}
INT getImageDWord(FILE* file)
{
	BYTE work[4];
	if(fread(work,4,1,file)==0)
		return 0;
	return (work[0] << 24) +(work[1] << 16) +(work[2] << 8)  + work[3];
}
bool getJpegHeader(LPCSTR fileName,IMAGEType* imageType)
{
	bool flag = false;
	INT code;
	...
	if(!file)
		return false;
	code = getImageWord(file);
	if(code == 0xffd8)
	{
		while(code = getImageWord(file))
		{
			if((code & 0xff00)!=0xff00)
				break;
			INT length = getImageWord(file)-2;

			if(code == 0xFFC0)
			{
				BYTE* buff = new BYTE[length];
				if(fread(buff,length,1,file) == 0)
				{
					delete[] buff;
					break;
				}
				imageType->sample = buff[0];
				imageType->height = (buff[1] << 8) + buff[2];
				imageType->width = (buff[3] << 8) + buff[4];
				imageType->colorType = buff[5];
				delete[] buff;
				flag = true;
				break;
			}
...
bool getPngHeader(LPCSTR fileName,IMAGEType* imageType)
{
	...
	if(!file)
		return false;
	BYTE buff[8];
	if(fread(buff,8,1,file) && memcmp(pngSignature,buff,8)==0)
	{
		INT length;
		while(length = getImageDWord(file))
		{
			DWORD code = getImageDWord(file);
			if(code == 0)
				break;
...
bool PDFImageObject::loadImage(LPCSTR fileName)
{
	INT kind;
	IMAGEType imageType;
	if(getJpegHeader(fileName,&imageType))
		kind = 0;
	else if(getPngHeader(fileName,&imageType))
		kind = 1;
	else
		return false;
...
}void StringData::loadImage(FLOAT x,FLOAT y,FLOAT width,FLOAT height,LPCSTR fileName)
{
...
void StringData::loadImage(LPCSTR fileName)
{
	IMAGEType imageType;
	if(!getJpegHeader(fileName,&imageType))
		if(!getPngHeader(fileName,&imageType))
			return;
...
bool Pdf::loadImage(BinaryStream* s,FLOAT x,FLOAT y,FLOAT width,FLOAT height,LPCSTR fileName)
{
	IMAGEType imageType;
	if(!getJpegHeader(fileName,&imageType))
		if(!getPngHeader(fileName,&imageType))
...
void Pdf::setStringData(PDFPagesObject* pages,StringData* sd)
{
	...
	for(i=0;src[i];i++)
	{
		...
		case 'I':
			getData(work,src,i);
			loadImage(stream,x,y,width,height,work.c_str());
			break;
		default:
			getData(work,src,i);
			break;
